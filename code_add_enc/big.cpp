...
/*--------------------------------------------------------------yk添加的素域上的模加函数-------------------------------------------------------------------*/
//nres_modadd(x,y,z)
Big modadd(const Big& b1,const Big& b2,const Big& m)
{Big z;prepare_monty(m.fn); nres_modadd(b1.fn,b2.fn,z.fn); return z;}
/*--------------------------------------------------------------yk添加的素域上的模加函数-------------------------------------------------------------------*/
/*--------------------------------------------------------------yk添加的素域上的模减函数-------------------------------------------------------------------*/
//nres_modadd(x,y,z)
Big modsub(const Big& b1,const Big& b2,const Big& m)
{Big z;prepare_monty(m.fn); nres_modsub(b1.fn,b2.fn,z.fn); return z;}
/*--------------------------------------------------------------yk添加的素域上的模减函数-------------------------------------------------------------------*/
/*--------------------------------------------------------------yk添加的素域上的模除函数-------------------------------------------------------------------*/
//nres_modadd(x,y,z)
...
{Big z; mad(b1.fn,b2.fn,b2.fn,m.fn,m.fn,z.fn); return z;}//yk添加的注释：void mad(x,y,z,w,q,r)函数是实现模乘、模加和模除的统一操作函数，其输出是：On exit q=(x.y+z)/w and r contains the remainder，
                                                         //且The addition of z is not done if x and z (or y and z) are the same.这样就解释了这个函数的功能是实现模乘操作。同理，令第2（或第一）项为1，
                                                         //即mad(b1.fn,1,b2.fn,m.fn,m.fn,z.fn)，则可实现模加操作;同理，令 x and z (or y and z) are the same，且第2项（或第1项）为1，
                                                         //即mad(b1.fn,1,b1.fn,m.fn,m.fn,z.fn)，则没能实现模除操作，只是实现了b1 mod m,如何利用mad函数实现模除操作？分子项上根本就没有除数，
                                                         //除非令第二项为1/b2.fn,且第3项同第一项，这样就得到了b1/b2/m

/*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
//当然，也可以分别实现模加减乘除，分别使用提供的函数：void nres_modadd(x,y,z)，void nres_modsub(x,y,z)，void nres_modmult(x,y,z)，void nres_moddiv(x,y,z)
/*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
