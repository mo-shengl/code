...
#ifdef ZZNS
#define MR_INIT_ECN                          \
    memset(mem, 0, mr_ecp_reserve(1, ZZNS)); \
    p = (epoint *)epoint_init_mem_variable(mem, 0, ZZNS);
#else
#define MR_INIT_ECN                \
    mem = (char *)ecp_memalloc(1); \
    p = (epoint *)epoint_init_mem(mem, 0);
#endif
...
public:
    ...
    ECn(const ECn &b)
    {
        MR_INIT_ECN epoint_copy(b.p, p);
    }

    epoint *get_point() const;
    int get_status() { return p->marker; }
    ECn &operator=(const ECn &b)
    {
        epoint_copy(b.p, p);
        return *this;
    }
    ...
    ECn &operator*=(const Big &k)
    {
        ecurve_mult(k.getbig(), p, p);
        return *this;
    }
...
#ifndef MR_SUPPORT_COMPRESSION
    BOOL set(const Big &x, int cb = 0)
    {
        return epoint_set(x.getbig(), x.getbig(), cb, p);
    }
#endif
    ...
    /*----------------�Լ����ӵ�������������:�������������ֵ�ܿ��ܲ�����Բ�����ϣ�һ����ֵ����һ��ECn���͵ı�������miracle��Ὣ��ֵ��ɣ�0,0��--------------------*/
    //friend  ECn operator^(const ECn& a,const ECn& b);
    //friend  ECn ecn_xor(const ECn& a,const ECn& b,const ECn& t);
    //friend void ecn_xor(const ECn& a,const ECn& b,Big& c, Big& d);
    friend void ecn_xor(Big &px, Big &py, Big &qx, Big &qy);
    /*---------------------------------------------------------------------�Լ����ӵ�������������-------------------------------------------------------------------------------------------------*/
    /*------------------------------------------------------------------  ---�Լ����ӵĵ�Ӻ���-------------------------------------------------------------------------------------------------*/
    friend ECn operator+(const ECn &, const ECn &);
    //{ return ecurve_add(b1.p,b2.p);}
...
/*------------------------------------------------------------------  ---�Լ����ӵĵ������-------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------  ---�Լ����ӵ�ȡָ��������꺯��-------------------------------------------------------------------------------------------------*/
//friend void ecngetxy(const ECn& b1, const big& b2,const big& b3);
//{return epoint_get(b1.p,b2,b3);}
/*------------------------------------------------------------------  ---�Լ����ӵ�ȡָ��������꺯��-------------------------------------------------------------------------------------------------*/
